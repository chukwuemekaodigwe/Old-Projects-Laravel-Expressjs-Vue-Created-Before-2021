...
<?php
            ...
            $status = '| <sub><i style="color:red; transform: rotate(-25deg);"> hidden </i></sub>';
        } else {
            $status = '';
        } 
?>
{!!$status!!}
                        ...
                        <div class="card-body flex-row align-items-center justify-content-between">
                            ...
                            <div class="col-12 col-md-12">
                                <span><strong> {{ $comment->guest_name}}  &nbsp; | &nbsp; ({{$comment->guest_email}})</strong> writes: </span>
                                <span style="float: right">
                                @empty($status)
                                    <form method="post" action="{{url('/dash/comments/'.$comment->id)}}" id="delete" class="form-horizontal">
                                        {{ csrf_field() }}
                                        {{ method_field('delete')}}
                                        ...
                                        <button type="submit" onclick="return $(this).submit()" class="btn btn-link">
                            ...
                            <blockquote style="font-family:'brush script mt' ; font-size: 1.2em;">{{ $comment->body }}</blockquote>
