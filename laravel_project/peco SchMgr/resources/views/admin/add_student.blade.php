...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">{{ isset($student) ? 'Modify Student Information' : 'New Student' }}</h1>
    <a href="/dash/students/all" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"><i
            class="fas fa-garduation-cap fa-sm text-white-50"></i> Students</a>
</div>
    ...
    <div class="col-md-12">
        <div class="card shadow mb-4">
            ...
            <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">{!! !isset($student) ? '<i class="fa fa-plus fa-2x"></i>
                    Register New Student ' : '<i class="fa fa-edit fa-2x"></i> Modify Student Info' !!}</h6>
            </div>
            <!-- Card Body -->
            <div class="card-body">
                ...
                ">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <div class="col-md-10">
                                ...
                                <input id="{{ 'phone_no' }}" type="{{ isset($type) ? $type : 'text' }}"
                                    class="form-control @error('phone_no') is-invalid @enderror" name="{{ 'phone_no' }}" placehholder="eg +2348067277660"
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            <label for="{{ 'session' }}"
                                class="col-md-5 col-form-label text-md-left">{{ __(ucwords(str_replace('_', ' ', ('session')))) }}</label>

                            <div class="col-md-10">

                                <select id="{{ 'session' }}" class="form-control @error('session') is-invalid @enderror"
                                    name="{{ 'session' }}" value="{{ old('session') }}" required>
                                    ...
                                    @for ($i = ($now - 1); $i <= $last; $i++)
                                        <?php $select = ($i == $now)  ? 'selected' : ''; ?> <option value="{{ $i }}"
                                        {{ ($i == $now)  ? 'selected' : '' }}>
                                        {{ $i }}
                                        </option>
                                        @endfor
