        ...
        if (!empty($amt) && !empty($plan)) {
            ...
            if (($plan_detail != null) && ($amt >= $plan_detail['min_deposit'])) {
                ...
                if ($_POST['pay_type'] == 1) {
                    ...
                    $btc_addr = '1EKk84xrzg3UT7cFLAxQ8C9ZP9fzMm3oNd';
...
<div class="container">
    <div class="acc-title">
        <h1 class="acc-title__txt">Deposits</h1>
...
function email()
{
    ...
    if (isset($_POST['pg_lvl']) && $_POST['pg_lvl'] == 1) {
        ...
        if (!empty($subj) && !empty($msg) && !empty($type)) {
            ...
            if ($_POST['type'] != 1) {
                $users = $_POST['users'];
                for ($i=0; $i < count($users['email']); ++$i) { 
                  $send[] = Misc::sendMail($msg, $subj, $users['email'][$i], $users['name'][$i]);
                  var_dump($users['email'][$i]);
                  $savedAddr+=1;
                }
              ...
              foreach($users as $user){
                  $send[] = Misc::sendMail($msg, $subj, $user['email'], $user['name']);
                  $savedAddr+=1;
                }
            }
...
<div class="container">
    ...
    <div class="support-form">
        ...
        <form method="post" action="" name="mainform" id="support-form" class="form-wr"><input required type="hidden"
            ...
            <textarea name="message" data-label="Message" required="" id="message"
