    ...
    <div class="plan-wr">
        <div class="container plan-wr__container">
            <div class="js-slider owl-carousel" style="display: flex; justify-content: space-around;">
                <?php
$plans = array();
    $plans = Transactions::getActiveInvestmentPlans();
    ...
    foreach ($plans as $plan) {
        ...
        echo ($plan['plan_id'] == 1) ? ' Day' : ' Days'; ?>
                    ...
                    <ul class="plan-card__list">
                        ...
                        <li>
                            <span>Max:</span>
                            <b><?php echo ($plan['plan_id'] != end($plans)['plan_id']) ? number_format($plan['max_deposit']) . ' USD' : 'Unlimited' ?>
                            </b>
                        </li>
                    </ul>
    ...
    // TO CALCULATE THEE OR TO CONVERT DAYS PF THE WEEK TO SECONDS
...
$data = time();
    ...
    $latest_deposits = Transactions::getLatestDeposits(0, 10);
//var_dump($latest_deposit);
    $latest_withd = array();
    $latest_withd = Transactions::getLatestWithdraw(0, 10);
...
$(document).ready(function(){
    setTimeout(load_deposit, 5000);
    setTimeout(load_withd, 7000);
});

function load_deposit(){
    $('#depositHolder').load('getTableData.php?data=deposit');
}

function load_withd(){
    $('#withdrawHolder').load('getTableData.php?data=withdraw');
}
...
<div class="logo"> <img src="' . $_SERVER['SERVER_NAME'] . '/assets/img/benefit.gif" alt="Crypto Benefit Inc "  height="100px" width="300px" />
...
<div style="
  ...
  <p>Thank you for signing up to Crypto Benefit!</p>
...
$from['email'] = 'info@'.$_SERVER['SERVER_NAME'];
                    $sent =   mailer::sendviaSwift($email, Users::getUserFullNameByEmail($email), $from, $subj1, strip_tags($msg), '', $attachment = null);
                    //echo '<script required type="text/javascript"> window.location = "secured/?pg=dash";</script>';
                    signup();
