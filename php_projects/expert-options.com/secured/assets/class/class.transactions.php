...
class Transactions
{
    ...
    public static function getInvestmentPlans()
    {
        ...
        $get = $db->query('SELECT * FROM investment_plans ORDER BY status DESC, plan_id DESC');
        $list = array();
    ...
    public static function getActiveInvestmentPlans()
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $get = $db->query('SELECT * FROM investment_plans WHERE status = 1 ORDER BY min_deposit ASC');
        $list = array();
    ...
    public static function saveNewPlan($name, $min, $max, $profit, $delay)
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $insert = $db->prepare('INSERT INTO investment_plans(name, min_deposit, max_deposit, profit, delay, status) VALUES(:urn, :min, :max, :profit, :delay, :status)');
        $insert->bindValue(':urn', ucwords($name), PDO::PARAM_STR);
        $insert->bindValue(':min', $min, PDO::PARAM_INT);
        $insert->bindValue(':max', $max, PDO::PARAM_INT);
        $insert->bindValue(':delay', $delay, PDO::PARAM_INT);
        $insert->bindValue(':profit', $profit, PDO::PARAM_INT);
    ...
    public static function updatePlan($name, $min, $max, $profit, $delay, $plan)
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $insert = $db->prepare('UPDATE investment_plans SET name = :name, min_deposit = :min, max_deposit = :max, profit = :profit, delay = :delay WHERE plan_id = :plan');
    ...
    public static function changePlanStatus($plan, $status)
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $get = $db->prepare('UPDATE investment_plans SET status = :status WHERE plan_id = :plan');
    ...
    public static function getLatestDeposits($start, $end)
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $get = $db->prepare('SELECT client_id,  amount, category, username FROM transaction WHERE type = 1 AND status = :status ORDER BY paymt_date DESC LIMIT :start, :end');
		$get->bindValue(':status', CONFIRM, PDO::PARAM_INT);
		$get->bindValue(':start', $start, PDO::PARAM_INT);
		$get->bindValue(':end', $end, PDO::PARAM_INT);
        $get->execute();
    ...
    public static function getLatestWithdraw($start, $end)
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $get = $db->prepare('SELECT client_id, amount, category, username FROM transaction WHERE type = 2 AND status = :status ORDER BY reg_date DESC LIMIT :start, :end');
		$get->bindValue(':status', CONFIRM, PDO::PARAM_INT);
		$get->bindValue(':start', $start, PDO::PARAM_INT);
		$get->bindValue(':end', $end, PDO::PARAM_INT);
        $get->execute();
    ...
    public static function addBTC($identifier, $address, $desc = '', $status)
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $insert = $db->prepare('INSERT INTO btc_address(identifier, btc_address, description, reg_date, status) VALUES(:urn, :btc, :desc, NOW(), :status)');
        $insert->bindValue(':urn', ucwords($identifier), PDO::PARAM_STR);
        $insert->bindValue(':btc', $address, PDO::PARAM_STR);
    ...
    public static function updateBTC($identifier, $new_address, $desc = '', $status, $id)
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $insert = $db->prepare('UPDATE btc_address SET identifier = :urn, btc_address = :address, description = :desc, status = :status WHERE id = :id');
        $insert->bindValue(':urn', ucwords($identifier), PDO::PARAM_STR);
        $insert->bindValue(':address', $new_address, PDO::PARAM_STR);
        $insert->bindValue(':desc', $desc, PDO::PARAM_STR);
        $insert->bindValue(':status', $status, PDO::PARAM_INT);
    ...
    public static function changeBTCStatus($id)
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $insert = $db->prepare('UPDATE btc_address SET status = :status WHERE id = :id');
        $insert->bindValue(':status', 1, PDO::PARAM_INT);
        $insert->bindValue(':id', $id, PDO::PARAM_INT);
        $insert->execute();
    ...
    public static function releaseBTCStatus()
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $insert = $db->prepare('UPDATE btc_address SET status = :status');
    ...
    public static function getBtcAddresses()
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $get = $db->prepare('SELECT * FROM btc_address ORDER BY status DESC, id DESC');
        $get->execute();
    ...
    public static function getActiveBtcAddress()
    {
        $db = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD, $_SESSION['dbErr']);
        $get = $db->prepare('SELECT btc_address FROM btc_address WHERE status = 1 LIMIT 1');
        $get->execute();

        $row = $get->fetch(PDO::FETCH_ASSOC);

        $result = $row['btc_address'];
