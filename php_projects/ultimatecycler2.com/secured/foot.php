...
$('#message_type').change(function(){
		...
		var exists = $('form').find('input#address').html();
		if(item === '2'){
			 if(exists != ''){
			
			$('div#address').slideDown('8000', function(){$('div#address').append('<label for="email-address"> Recipients Address</label><input value="" required type="text" class="input" placeholder="Enter the clients email address each separated by semi-colon" title="Enter the clients email address each separated by semi-colon" name="addr" id="address">');});
		}
		}else{
			if(exists == ''){
				$('div#address').html('');
